...
namespace nothinbutdotnetstore.tasks.stubs
{
    class StubViewMainDepartmentTasks : CatalogTasks
    {
        ...
        public IEnumerable<ProductItem> get_products_in_department(DepartmentItem department_item)
        {
            return Enumerable.Range(1, 10).Select(i => new ProductItem { id = i.ToString(), name = i.ToString("Sub department 0" + " for department " + department_item.name) });
        }
    }
}